...
- Kimi K2.5 had the highest hard recall in `6` of `9` hard-threshold configurations, whereas DeepSeek-R1 preserved the strongest fidelity-balanced profile because its higher directional control is not driven by a sparse graph.
- Llama 3.1-8B remained the weakest coverage model across all sweeps; discriminability is unchanged because SNR and detection power are threshold-independent.
